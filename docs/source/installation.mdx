...
### PyTorch CUDA variant (Linux and Windows)

Source installs on Linux and Windows use the CUDA 12.8 PyTorch index by default. macOS uses the PyPI default with MPS support and can skip this section.
...
On Linux and Windows, `torch` and `torchvision` are pinned by the project to the **CUDA 12.8** PyTorch index (`https://download.pytorch.org/whl/cu128`, driver floor **570.86**) — covers Ampere/Ada/Hopper/Blackwell GPUs. No action needed for typical NVIDIA setups.
